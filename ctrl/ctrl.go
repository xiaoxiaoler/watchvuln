...
func (w *WatchVulnApp) startHTTPServer(ctx context.Context) error {
	if w.config.HTTPListen == "" {
		return nil
	}

	mux := http.NewServeMux()
	mux.HandleFunc("/", w.handleIndexPage)
	mux.HandleFunc("/api/vulns", w.handleVulnsAPI)
	// 添加静态文件服务
	mux.Handle("/static/", http.StripPrefix("/static/", http.FileServer(http.Dir("static"))))

	w.httpServer = &http.Server{
		Addr:    w.config.HTTPListen,
		Handler: mux,
	}
	...
	go func() {
		...
		if err := w.httpServer.Shutdown(shutdownCtx); err != nil {
			w.log.Warnf("http server shutdown error: %v", err)
		}
	}()

	go func() {
		w.log.Infof("http server listening on %s", w.config.HTTPListen)
...
func (w *WatchVulnApp) handleVulnsAPI(rw http.ResponseWriter, r *http.Request) {
	ctx, cancel := context.WithTimeout(r.Context(), time.Second*10)
	defer cancel()

	search := r.URL.Query().Get("search")
	disclosureStart := r.URL.Query().Get("disclosure_start")
	disclosureEnd := r.URL.Query().Get("disclosure_end")

	updateStartStr := r.URL.Query().Get("update_start")
	updateEndStr := r.URL.Query().Get("update_end")

	var updateStart, updateEnd time.Time
	if updateStartStr != "" {
		if t, err := time.Parse("2006-01-02", updateStartStr); err == nil {
			updateStart = t
		}
	}
	if updateEndStr != "" {
		if t, err := time.Parse("2006-01-02", updateEndStr); err == nil {
			// 设置为当天结束时间
			updateEnd = t.Add(24*time.Hour - time.Second)
		}
	}

	var pushed *bool
	if pushedStr := r.URL.Query().Get("pushed"); pushedStr != "" {
		p := pushedStr == "true"
		pushed = &p
	}
...
func (w *WatchVulnApp) queryAllVulns(ctx context.Context, search string, disclosureStart, disclosureEnd string, updateStart, updateEnd time.Time, pushed *bool, severities string) ([]*ent.VulnInformation, error) {
	...
	// 披露时间范围筛选
	if disclosureStart != "" {
		query = query.Where(vulninformation.DisclosureGTE(disclosureStart))
	}
	if disclosureEnd != "" {
		query = query.Where(vulninformation.DisclosureLTE(disclosureEnd))
	}

	// 更新时间范围筛选
	if !updateStart.IsZero() {
		query = query.Where(vulninformation.UpdateTimeGTE(updateStart))
	}
	if !updateEnd.IsZero() {
		query = query.Where(vulninformation.UpdateTimeLTE(updateEnd))
	}
